...
class StatisticsService:
    ...
    def update_company_statistics(company_id, db_session):
        ...
        try:
            ...
            stats = db.session.query(Statistics).filter(
    ...
    def update_user_statistics(user_id, db_session):
        ...
        try:
            ...
            # Actions count
            action_count = db.session.query(func.count(Log.id)).filter(
                Log.user_id == user_id,
            ...
            if user.role == UserRole.MANAGER:
                ...
                operator_count = db.session.query(func.count(Operator.id)).filter(
                    Operator.manager_id == user.id
                ).scalar()
                metrics['operator_count'] = operator_count

                # Count tasks created by this manager
                task_count = db.session.query(func.count(Task.id)).filter(
                ...
                driver_count = db.session.query(func.count(Driver.id)).filter(
                ...
                route_count = db.session.query(func.count(Route.id)).filter(
                ...
                completed_routes = db.session.query(func.count(Route.id)).filter(
                    Route.driver_id == user.id,
